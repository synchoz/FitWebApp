...
export default function AdminFoodCatalog() {
    ...
    const numberInput = (value, onChange, width = 'w-16') => (
        <input
            ...
            value={value}
            onChange={onChange}
            className={`border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-2 py-0.5 ${width} text-sm transition-colors focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100`}
        />
    );

    return (
        <div className='bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5'>
            <div className='text-sm text-gray-500 dark:text-gray-400 mb-3'>Food Catalog</div>
            ...
            <form onSubmit={handleAdd} className='grid grid-cols-2 sm:flex sm:flex-wrap sm:items-end gap-3 mb-4 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl p-3'>
                <div className='col-span-2 sm:col-auto flex flex-col'>
                    ...
                    <input
                        ...
                        className='border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 rounded-xl px-3 py-2 text-sm w-full sm:min-w-[160px] transition-colors focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100'
                        placeholder='e.g. Banana'
                    />
                </div>
                {['calories', 'protein', 'carbs', 'fats', 'amount'].map((field) => (
                    <div key={field} className='flex flex-col'>
                        <label className='text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 capitalize'>{field}</label>
                        <input
                            type='number'
                            value={newFood[field]}
                            onChange={(e) => setNewFood({ ...newFood, [field]: e.target.value })}
                            className='border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 rounded-xl px-3 py-2 text-sm w-20 transition-colors focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100'
                        />
                    </div>
                ))}
                <button
                ...
                <table className='w-full text-sm whitespace-nowrap'>
                    ...
                    <tbody className='text-gray-700 dark:text-gray-200'>
                        {foods.map((row) => {
                            ...
                            return (
                                <tr key={row.food} className='border-b border-gray-100 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors'>
                                    <td className='py-2 font-medium text-gray-800 dark:text-gray-100'>{row.food}</td>
